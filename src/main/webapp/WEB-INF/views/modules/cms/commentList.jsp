...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctxAdmin}/cms/comment/">评论列表</a></li><%--
		<e:hasPermission name="cms:comment:edit"><li><a href="${ctxAdmin}/cms/comment/form?id=${comment.id}">评论添加</a></li></e:hasPermission> --%>
	</ul>
	<form:form id="searchForm" modelAttribute="comment" action="${ctxAdmin}/cms/comment/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.result}" var="comment">
			<tr>
				...
				<td><e:hasPermission name="cms:comment:edit">
					<c:if test="${comment.status ne '2'}"><a href="${ctxAdmin}/cms/comment/delete?id=${comment.id}${comment.status ne 0?'&isRe=true':''}"
						onclick="return confirmx('确认要${comment.status ne 0?'恢复审核':'删除'}该审核吗？', this.href)">${comment.status ne 0?'恢复审核':'删除'}</a></c:if>
					<c:if test="${comment.status eq '2'}"><a href="${ctxAdmin}/cms/comment/_save?id=${comment.id}">审核通过</a></c:if></e:hasPermission>
				</td>
			</tr>
