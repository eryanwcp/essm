...
<head>
	...
	<script type="text/javascript">
        ...
        function contentFileChange() {
            var fileUrl = $("#contentFile").val();
            $("#content").load(fileUrl, {jsonFile: 1}, function (response, status, xhr) {
        ...
        function uploadify() {
            $('#file').uploadify({
                ...
                buttonText: '上传.doc文件',                                 //按钮文本
                uploader: '${ctxAdmin}/cms/article/importDoc;jsessionid=<%=session.getId()%>',
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctxAdmin}/cms/article/?category.id=${article.category.id}">文章列表</a></li>
		<li class="active"><a href="<c:url value='${fns:getAdminPath()}/cms/article/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value=''/></c:url>">文章<e:hasPermission name="cms:article:edit">${not empty article.id?'修改':'添加'}</e:hasPermission><e:lacksPermission name="cms:article:edit">查看</e:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="article" action="${ctxAdmin}/cms/article/_save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<form:select path="color" class="input-mini">
					...
					<form:options items="${fns:getDictList('color')}" itemLabel="name" itemValue="value" htmlEscape="false" />
				</form:select>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
                ...
                <span class="help-inline">建议首页轮播图大小：1600 × 410（像素）</span>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">缩略图:</label>
			<div class="controls">
                <input type="hidden" id="thumb" name="thumb" value="${article.thumbSrc}" />
				<tags:ckfinder input="thumb" type="thumb" uploadPath="/cms/article" selectMultiple="false"/>
                <span class="help-inline">建议首页推荐图大小：115 × 167（像素）</span>
			</div>
		</div>
        <div class="control-group">
            <label class="control-label">正文导入:</label>

            <div class="controls">
                <input type="hidden" id="contentFile"/>
			...
			<div class="controls">
				...
				<script type="text/javascript">
					...
					$("#relationButton").click(function(){
						top.$.jBox.open("iframe:${ctxAdmin}/cms/article/selectList?pageSize=8", "添加相关",$(top.document).width()-220,$(top.document).height()-180,{
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:radiobuttons path="articleData.allowComment" items="${fns:getDictList('yes_no')}" itemLabel="name" itemValue="value" htmlEscape="false"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">推荐位:</label>
			<div class="controls">
				<form:checkboxes path="posidList" items="${fns:getDictList('cms_posid')}" itemLabel="name" itemValue="value" htmlEscape="false"/>
			</div>
		</div>
		<div class="control-group">
		...
		<e:hasPermission name="cms:article:audit">
			<div class="control-group">
				...
				<div class="controls">
					<form:radiobuttons path="status" items="${fns:getDictList('cms_del_flag')}" itemLabel="name" itemValue="value" htmlEscape="false" class="required"/>
		...
		<div class="form-actions">
			<e:hasPermission name="cms:article:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</e:hasPermission>
