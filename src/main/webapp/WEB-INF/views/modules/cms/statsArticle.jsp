...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctxAdmin}/cms/stats/article">信息量统计</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="article" action="${ctxAdmin}/cms/stats/article" method="post" class="breadcrumb form-search">
		<div>
			...
			<label>归属机构：</label><tags:treeselect id="organ" name="organId" value="${paramMap.organ}" labelName="organName" labelValue="${paramMap.organName}"
				title="机构" url="/sys/organ/treeData" cssClass="input-small" allowClear="true"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${list}" var="stats">
			<tr>
				...
				<td>${stats.cnt}</td>
				<td>${stats.hits}</td>
				<td><fmt:formatDate value="${stats.updateTime}" type="both"/></td>
				<td><a href="javascript:" onclick="$('#organId').val('${stats.organ}');$('#organName').val('${stats.organName}');$('#searchForm').submit();return false;">${stats.organName}</a></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
