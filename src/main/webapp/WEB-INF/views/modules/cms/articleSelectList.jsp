...
<body>
	...
	<form:form id="searchForm" modelAttribute="article" action="${ctxAdmin}/cms/article/selectList" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.result}" var="content">
			<tr>
				<td style="text-align:center;"><input type="checkbox" name="id" value="${content.id}" title="${fns:abbr(content.title,40)}" /></td>
				...
				<td><a href="${ctxAdmin}/cms/article/form?id=${content.id}" title="${content.title}" onclick="return view(this.href);">${fns:abbr(content.title,40)}</a></td>
