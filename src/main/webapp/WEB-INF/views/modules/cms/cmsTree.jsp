...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
            ...
            <%--栏目权限--%>
			var zNodes=[
                    <c:forEach items="${categoryList}" var="tpl"><c:if test="${fns:isPermitted(tpl.id)}">{id:'${tpl.id}', pId:'${not empty tpl.parent?tpl.parent.id:0}', name:"${tpl.name}", url:"${ctxAdmin}/cms/${not empty tpl.module?tpl.module:'none'}/?category.id=${tpl.id}", target:"cmsMainFrame"},
                </c:if></c:forEach>];
