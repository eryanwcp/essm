...
<div class="container">
<dl class="map">
    <c:forEach items="${fnc:getMainNavList(site.code)}" var="tpl">
        <dt>
            <c:choose>
        ...
        <dd>
            <c:forEach items="${fnc:getCategoryList(site.code, tpl.id, -1, '')}" var="category">
                <c:choose>
                    <c:when test="${not empty category.href}">
                        <c:choose>
                            <c:when test="${fn:indexOf(category.href, '://') eq -1}"><c:set var="url" value="${ctxFront}${category.href}"/></c:when>
                            <c:otherwise><c:set var="url" value="${category.href}"/></c:otherwise>
                        </c:choose>
                    </c:when>
                    <c:otherwise><c:set var="url" value="${ctxFront}/list-${category.code}${urlSuffix}"/></c:otherwise>
                </c:choose>
