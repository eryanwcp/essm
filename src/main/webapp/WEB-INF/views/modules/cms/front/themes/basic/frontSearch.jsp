	...
	<dl class="search">
		<c:if test="${empty t || t eq 'article'}">
			<c:forEach items="${page.result}" var="content">
				<dt><a href="${ctxFront}/view-${content.category.code}-${content.id}${urlSuffix}" class="title" target="_blank">${content.title}</a></dt>
				<dd>${content.description}<span class="info"><br/>发布者：${content.createUser} &nbsp; 点击数：${content.hits} &nbsp; 发布时间：<fmt:formatDate value="${content.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/> &nbsp; 更新时间：<fmt:formatDate value="${content.updateTime}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
					&nbsp;&nbsp;<a href="${ctxFront}/view-${content.category.code}-${content.id}${urlSuffix}" target="_blank">查看全文</a><br/></dd>
			</c:forEach>
		</c:if>
		<c:if test="${t eq 'guestbook'}">
			<c:forEach items="${page.result}" var="guestbook"><dt>${fns:getDictionaryNameByDV('cms_guestbook',guestbook.type,'')}</dt>
				<dd>${guestbook.content}<span class="info"><br/>姓名：${guestbook.name} &nbsp; 留言时间：<fmt:formatDate value="${guestbook.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></span></dd>
