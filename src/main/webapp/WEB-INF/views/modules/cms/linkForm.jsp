...
<ul class="nav nav-tabs">
	<li><a href="${ctxAdmin}/cms/link/?category.id=${link.category.id}">链接列表</a></li>
	<li class="active"><a href="<c:url value='${fns:getAdminPath()}/cms/link/form?id=${link.id}&category.id=${link.category.id}'><c:param name='category.name' value='${link.category.name}'/></c:url>">链接<e:hasPermission name="cms:link:edit">${not empty link.id?'修改':'添加'}</e:hasPermission><e:lacksPermission name="cms:link:edit">查看</e:lacksPermission></a></li>
</ul><br/>
<form:form id="inputForm" modelAttribute="link" action="${ctxAdmin}/cms/link/_save" method="post" class="form-horizontal">
	<form:hidden path="id"/>
	...
	<div class="form-actions">
		<e:hasPermission name="cms:link:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</e:hasPermission>
