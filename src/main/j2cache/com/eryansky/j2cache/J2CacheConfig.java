...
public class J2CacheConfig {
    ...
    public final static J2CacheConfig initFromConfig(String configResource) throws IOException {
        ...
        try (InputStream stream = getConfigStream(configResource)){
            ...
            Set<Object> keySet = config.properties.keySet();
            for(Object k:keySet){
                String key = (String)k;
                String v = config.properties.getProperty(key);
                if(key.startsWith(config.broadcast + "."))
                    config.broadcastProperties.setProperty(key.substring((config.broadcast + ".").length()), v);
                if(key.startsWith(config.l1CacheName + "."))
                    config.l1CacheProperties.setProperty(key.substring((config.l1CacheName + ".").length()), v);
                if(key.startsWith(l2_section + "."))
                    config.l2CacheProperties.setProperty(key.substring((l2_section + ".").length()), v);
            }
        }
        return config;
    }
