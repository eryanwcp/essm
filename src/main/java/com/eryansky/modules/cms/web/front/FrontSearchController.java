...
public class FrontSearchController extends SimpleController {

	@Autowired
	private ArticleService articleService;
	@Autowired
	private ArticleSeachService articleSeachService;
	@Autowired
	...
	public String search(String t, @RequestParam(required=false) String q, @RequestParam(required=false) String qand, @RequestParam(required=false) String qnot,
		...
		if ("cmd:reindex".equals(q)){
			if (userManager.isSuperUser(SecurityUtils.getCurrentUser().getId())){
				// 文章模型
		...
		// 执行检索
			...
			// 文章检索
			if (StringUtils.isBlank(t) || "article".equals(t)){
				Page<Article> page = articleSeachService.search(new Page<Article>(request, response), qStr, cid, bd, ed);
